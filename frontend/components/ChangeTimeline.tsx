...
function TimelineItem({
  ...
  return (
    <div className="relative pl-8 pb-8 border-l border-border last:border-l-0 last:pb-0">
      {/* Timeline dot */}
      <div className="absolute -left-[5px] top-0 w-2.5 h-2.5 bg-sage-600 border-2 border-paper" />

      {/* Content */}
      <div
        className="bg-paper border border-border p-4 hover:border-sage-600 transition-colors cursor-pointer"
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-wrap gap-1">
            {update.affected_sections.slice(0, 3).map((section) => (
              <span
                key={section}
                className="px-2 py-0.5 bg-surface border border-border text-xs font-mono text-ink-700"
              >
                {section}
              </span>
            ))}
            {update.affected_sections.length > 3 && (
              <span className="px-2 py-0.5 bg-surface border border-border text-xs font-mono text-ink-500">
                +{update.affected_sections.length - 3}
              </span>
            )}
          </div>
      ...
      <div className="mb-6 flex items-center gap-4">
        ...
        <div className="flex gap-1">
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium transition-colors ${
              filter === 'all'
                ...
                : 'bg-surface border border-border text-ink-700 hover:border-sage-600'
            }`}
          >
            All
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium transition-colors ${
              filter === 'fannie_mae'
                ? 'bg-fannie text-white'
                : 'bg-surface border border-border text-ink-700 hover:border-fannie'
            }`}
          >
            Fannie Mae
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium transition-colors ${
              filter === 'freddie_mac'
                ? 'bg-freddie text-white'
                : 'bg-surface border border-border text-ink-700 hover:border-freddie'
            }`}
          >
            Freddie Mac
          </button>
